		...
		jQuery(document).ready(function() {
			jQuery('#exlude_dirs').fileTree({ root: '/', multiFolder: false, script: '<?php echo admin_url('admin-ajax.php').'?action=pb_backupbuddy_filetree'; ?>' }, function(file) {
				...
				if ( ( directory == '/wp-content/' ) || ( directory == '/wp-content/uploads/' ) || ( directory == '/wp-content/uploads/backupbuddy_backups/' ) ) {
					alert( '<?php _e('You cannot exclude /wp-content/ or /wp-content/uploads/.  However, you may exclude subdirectories within these. The backupbuddy_backups directory is also automatically excluded and cannot be added to exclusion list.', 'it-l10n-backupbuddy');?>' );
				} else {
				...
				<div class="meta-box-sortables">
					...
					<div id="breadcrumbslike" class="postbox">
						...
						<div class="inside">
							<table class="form-table">
								<tr>
									<td><label for="email_notify_scheduled"><?php _e('Scheduled backup completion recipients', 'it-l10n-backupbuddy'); $this->tip( __('Email address(es) to send notifications to upon scheduled backup completion.', 'it-l10n-backupbuddy') ); ?></label></td>
								...
								<tr>
									<td><label for="email_notify_manual"><?php _e('Manual backup completion recipients', 'it-l10n-backupbuddy'); $this->tip( __('Email address(es) to send notifications to upon manually triggered backup completion.', 'it-l10n-backupbuddy') ); ?></label></td>
								...
								<tr>
									<td><label for="email_notify_error"><?php _e('Backup failure/error recipients','it-l10n-backupbuddy'); $this->tip( __('Email address(es) to send notifications to upon encountering any errors or problems.', 'it-l10n-backupbuddy') ); ?></label></td>
					...
					<div id="breadcrumbslike" class="postbox">
						...
						<div class="inside">
							<table class="form-table">
								<tr>
									<td><label for="import_password"><?php _e('Import password (optional)', 'it-l10n-backupbuddy'); $this->tip( __('[Example: myp@ssw0rD] - Optional password to be required to launch the importbuddy import/migration script. This prevents unauthorized users from being able to view your backup archive listing on the destination server during Step 1 or proceed with an unauthorized import.', 'it-l10n-backupbuddy') ); ?></label></td>
								...
								<tr>
									...
									<td>
										...
										<label for="backup_nonwp_tables">
											<?php _e('Enable backing up all tables in your database (not just WordPress tables)','it-l10n-backupbuddy');?>
											<br />
								...
								<tr>
									...
									<td>
										...
										<label for="compression">
											...
											<br />
											<span style="color: #AFAFAF; margin-left: 26px;"><?php _e('Disable this if your site is too large, causing backups to not complete.', 'it-l10n-backupbuddy'); ?></span>
										</label>
										<br />
								...
								/*
								<tr>
									<td><?php esc_html_e( 'Enable RepairBuddy?', 'it-l10n-backupbuddy' ); ?> (BETA)</td>
									<td>
										<select name="#enable_repair_buddy">
											<option value="1" <?php if ( $this->_options['enable_repair_buddy'] == '1' ) { echo 'selected'; } ?>><?php _e('Yes', 'it-l10n-backupbuddy');?></option>
											<option value="2" <?php if ( $this->_options['enable_repair_buddy'] == '2' ) { echo 'selected'; } ?>><?php _e('No', 'it-l10n-backupbuddy');?></option>
										</select>
									</td>
									</tr>
					...
					<div id="breadcrumbslike" class="postbox">
						...
						<div class="inside">
							<table class="form-table">
								<tr>
									<td>
										<?php echo sprintf( __('Click directory to navigate to it or %s to exclude it.', 'it-l10n-backupbuddy'), '<img src="' . $this->_pluginURL .'/images/bullet_delete.png" style="vertical-align: -3px;" />'); $this->tip( __('Click on a directory name to navigate directories. Click the red minus sign to the right of a directory to place it in the exclusion list. /wp-content/ and /wp-content/uploads/ cannot be excluded.', 'it-l10n-backupbuddy' ) ); ?><br />
