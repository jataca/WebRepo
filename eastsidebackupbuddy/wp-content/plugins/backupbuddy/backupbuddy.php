	...
	class pluginbuddy_backupbuddy {
		...
		function pluginbuddy_backupbuddy() {
			...
			if ( is_admin() ) { // Admin part of site.
				...
				add_action( 'wp_ajax_backupbuddy_importbuddy_beta', array( &$this, 'ajax_importbuddy_beta' ) ); // Direct downloading of importbuddy.php using ajax portal.
				add_action( 'wp_ajax_backupbuddy_repairbuddy_beta', array( &$this, 'ajax_repairbuddy_beta' ) ); // Direct downloading of repairbuddy.php using ajax portal.
		...
		function send_remote_destination( $destination_id, $file ) {
			...
			if ( $destination['type'] == 's3' ) {
				...
				$response = $this->remote_send_rackspace( $destination['username'], $destination['api_key'], $destination['container'], $file, $destination['archive_limit'] );
		...
		function ajax_importbuddy_beta() {
			if ( defined( 'PB_DEMO_MODE' ) ) {
				die( 'Access denied in demo mode.' );
			}
			
			$this->load();
			$output = file_get_contents( dirname( __FILE__ ) . '/importbuddy_beta.php' );
		...
		function ajax_repairbuddy_beta() {
			if ( defined( 'PB_DEMO_MODE' ) ) {
				die( 'Access denied in demo mode.' );
			}
			
			$this->load();
			$output = file_get_contents( dirname( __FILE__ ) . '/repairbuddy_beta.php' );
		...
		function remote_send_dropbox( $token, $directory, $file, $limit = 0 ) {
			...
			if ( $limit > 0 ) {
				...
				if ( ( count( $backups ) ) > $limit ) {
					...
					if ( $delete_count !== 0 ) {
						$this->mail_error( sprintf( __('Dropbox remote limit could not delete %s backups.', 'it-l10n-backupbuddy'), $delete_count) );
					}
				}
			}
			...
			if ( $s3->putObject( S3::inputFile( $file ), $bucket, $directory . basename( $file ), S3::ACL_PRIVATE) ) {
				...
				if ( $limit > 0 ) {
					...
					if ( ( count( $backups ) ) > $limit ) {
						...
						if ( $delete_count !== 0 ) {
							$this->mail_error( sprintf( __('Amazon S3 remote limit could not delete %s backups.', 'it-l10n-backupbuddy'),  $delete_count ) );
						}
					}
				}
				// End remote backup limit
				...
				$this->mail_error( __('ERROR #9002! Failed sending file to Amazon S3.', 'it-l10n-backupbuddy') );
		...
		function remote_send_rackspace( $rs_username, $rs_api_key, $rs_container, $rs_file, $limit = 0 ) {
			...
			if ( $testbackup->load_from_filename( ABSPATH . 'wp-content/uploads/backupbuddy_backups/' . $rs_file ) ) {
				...
				if ( $limit > 0 ) {
					...
					if ( ( count( $backups ) ) > $limit ) {
						...
						if ( $delete_count !== 0 ) {
							$this->mail_error( sprintf( __('Rackspace remote limit could not delete %s backups.', 'it-l10n-backupbuddy'), $delete_count  ) );
						}
					}
				}
				// End remote backup limit
				...
				$this->mail_error( __('ERROR #9002-rs! Failed sending file to Rackspace.', 'it-l10n-backupbuddy') );
			...
			if ( $upload === false ) {
				...
				if ( $limit > 0 ) {
					...
					if ( ( count( $results ) ) > $limit ) {
						...
						if ( $delete_count !== 0 ) {
							$this->mail_error( sprintf( __('FTP remote limit could not delete %s backups.', 'it-l10n-backupbuddy'), $delete_count  ) );
						}
					}
				}
				// End remote backup limit
		...
		function test_rackspace( $rs_username, $rs_api_key, $rs_container ) {
			...
			if ( false === ( $container = @$conn->get_container( $rs_container ) ) ) {
				return __('Invalid container. You must create it first.', 'it-l10n-backupbuddy');
			}
			// Create test file
