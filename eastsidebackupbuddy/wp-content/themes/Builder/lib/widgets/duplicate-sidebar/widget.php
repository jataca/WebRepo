...
if ( ! class_exists( 'ITDuplicateSidebarWidgets' ) ) {
	class ITDuplicateSidebarWidgets extends WP_Widget {
		...
		function ITDuplicateSidebarWidgets() {
			$widget_ops = array( 'classname' => 'it_duplicate_sidebar', 'description' => __( 'Duplicate another sidebar&#8217;s widgets', 'it-l10n-Builder' ) );
			$control_ops = array( 'width' => 350 );
			
			$this->WP_Widget( 'it_duplicate_sidebar', __( 'Duplicate Sidebar', 'it-l10n-Builder' ), $widget_ops, $control_ops );
		...
		function form( $instance ) {
			...
			asort( $this->_registered_sidebars );
			
?>
	<p>
		<label for="<?php echo $this->get_field_id( 'sidebar' ); ?>"><?php _e( 'Sidebar to Duplicate:', 'it-l10n-Builder' ); ?></label><br />
...
<?php
		...
		function store_registered_sidebars() {
			...
			foreach ( (array) $wp_registered_sidebars as $sidebar )
				$this->_registered_sidebars[$sidebar['id']] = $sidebar['name'];
			
			ksort( $this->_registered_sidebars );
