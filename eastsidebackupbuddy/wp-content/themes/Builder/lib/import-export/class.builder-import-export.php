...
if ( ! class_exists( 'BuilderImportExport' ) ) {
	class BuilderImportExport {
		...
		function get_export( $name, $data_sources = array() ) {
			...
			$theme_data = get_theme_data( dirname( dirname( dirname( __FILE__ ) ) ) . '/style.css' );
			...
			$info = array(
				'name'					=> $name,
				'guid'					=> $this->_guid,
				'builder_version'		=> $theme_data['Version'],
			...
			foreach ( (array) $data_sources as $var ) {
				...
				$zip->add_content( $data, "data-sources/$var.txt" );
		...
		function show_data_source_import_methods( $form ) {
			...
			foreach ( (array) $info['data_sources'] as $var => $data_source ) {
				...
				if ( isset( $this->_data_sources[$var] ) && is_callable( array( &$this->_data_sources[$var], 'show_import_methods_form' ) ) )
					...
					echo "<p>" . __( 'The plugin or component that this data is for is not present on this site. This data will not be imported.', 'it-l10n-Builder' ) . "</p>\n";
			...
			foreach ( (array) $info['data_sources'] as $var => $data_source ) {
				...
				if ( isset( $this->_data_sources[$var] ) ) {
					if ( 'skip' === $submission_data['data_sources'][$var]['method'] )
						echo '<p>' . sprintf( __( 'The %1$s data from the export file will not be imported to the site.', 'it-l10n-Builder' ), $this->_data_sources[$var]->get_name() ) . "</p>\n";
					else {
						...
						if ( false === $has_custom_process ) {
							if ( 'replace' === $submission_data['data_sources'][$var]['method'] )
								printf( __( 'The %1$s data on the site will be removed and replaced with the %1$s data from the export file.', 'it-l10n-Builder' ), $this->_data_sources[$var]->get_name() );
							else
								_e( 'The data will be imported as selected.', 'it-l10n-Builder' );
						}
					}
			...
			foreach ( (array) $info['data_sources'] as $var => $data_source ) {
				if ( isset( $this->_data_sources[$var] ) ) {
					if ( is_callable( array( $this->_data_sources[$var], 'run_import' ) ) ) {
						$result = $this->_data_sources[$var]->run_import( $info['data_sources'][$var], $this->get_data( $var ), $submission_data['data_sources'][$var], $new_attachments );
		...
		function _get_file_data( $file ) {
			...
			if ( ! is_file( "{$this->_file_path}/$file" ) )
				return false;
			
			return unserialize( file_get_contents( "{$this->_file_path}/$file" ) );
		}
		...
		function _import_file( $file ) {
			...
			if ( ! is_file( $file ) ) {
				$this->add_error( new WP_Error( 'builder-import-export-no-file', __( 'Unable to find requested export file', 'it-l10n-Builder' ) ) );
				return false;
			}
			...
			if ( is_wp_error( $result ) || ! is_file( "$path/info.txt" ) || ! is_dir( "$path/data-sources" ) ) {
				...
				$this->add_error( new WP_Error( 'builder-import-export-invalid-export-format', __( 'The uploaded file is not a valid Builder export file. It cannot be imported.', 'it-l10n-Builder' ) ) );
				return false;
			}
			
			
			$raw_info = file_get_contents( "$path/info.txt" );
