...
if ( ! class_exists( 'BuilderHeaderWidget' ) ) {
	class BuilderHeaderWidget extends WP_Widget {
		function BuilderHeaderWidget() {
			$widget_ops = array( 'classname' => 'widget-builder-header', 'description' => __( 'Add site title and description to your site', 'it-l10n-Builder' ) );
			$control_ops = array( 'width' => 300 );
			
			$this->WP_Widget( 'it_builder_header_widget', __( 'Header', 'it-l10n-Builder' ), $widget_ops, $control_ops );
		}
...
<?php
		...
		function form( $instance ) {
			...
			$use_default_tag_settings_options = array(
				'1' => __( 'Yes' ),
				''  => __( 'No, use custom tag settings', 'it-l10n-Builder' ),
			);
...
?>
	<p>
		<label for="<?php echo $this->get_field_id( 'show_site_title' ); ?>"><?php printf ( __( 'Show Site Title (configured in <a href="%s">Settings > General</a>):', 'it-l10n-Builder' ), admin_url( 'options-general.php' ) ); ?></label><br />
		<?php $form->add_yes_no_drop_down( 'show_site_title' ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'custom_site_title' ); ?>"><?php _e( 'Customize Site Title (leave blank for default Site Title):', 'it-l10n-Builder' ); ?></label><br />
		<?php $form->add_text_box( 'custom_site_title' ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'show_tagline' ); ?>"><?php printf ( __( 'Show Tagline (configured in <a href="%s">Settings > General</a>):', 'it-l10n-Builder' ), admin_url( 'options-general.php' ) ); ?></label><br />
		<?php $form->add_yes_no_drop_down( 'show_tagline' ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'custom_tagline' ); ?>"><?php _e( 'Customize Tagline (leave blank for default Tagline):', 'it-l10n-Builder' ); ?></label><br />
		<?php $form->add_text_box( 'custom_tagline' ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'home_title_tag' ); ?>"><?php _e( 'Site Title tag (when on the home page):', 'it-l10n-Builder' ); ?></label><br />
		<?php $form->add_drop_down( 'home_title_tag', $home_title_tag_options ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'home_tagline_tag' ); ?>"><?php _e( 'Tagline tag (when on the home page):', 'it-l10n-Builder' ); ?></label><br />
		<?php $form->add_drop_down( 'home_tagline_tag', $generic_tag_options ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'site_title_tag' ); ?>"><?php _e( 'Site Title tag (on other site views):', 'it-l10n-Builder' ); ?></label><br />
		<?php $form->add_drop_down( 'site_title_tag', $generic_tag_options ); ?>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'site_tagline_tag' ); ?>"><?php _e( 'Tagline tag (on other site views):', 'it-l10n-Builder' ); ?></label><br />
