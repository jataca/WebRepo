...
if ( ! class_exists( 'BuilderExtensions' ) ) {
	class BuilderExtensions {
		...
		function BuilderExtensions() {
			add_action( 'builder_layout_engine_identified_layout', array( &$this, 'enable_extension' ), 10, 2 );
			add_action( 'builder_layout_engine_identified_view', array( &$this, 'enable_extension' ), 10, 2 );
		...
		function enable_extension( $layout_id = null, $data = array() ) {
			if ( ! empty( $this->_active_extension ) )
				return;
			
			
			if ( 'builder_layout_engine_identified_layout' == current_filter() ) {
				if ( ! is_array( $data ) || empty( $data['layouts'][$layout_id]['extension'] ) )
					return;
				
				$extension = $data['layouts'][$layout_id]['extension'];
				$disable_style = $data['layouts'][$layout_id]['extension_disables_theme_style'];
			}
			else if ( 'builder_layout_engine_identified_view' == current_filter() ) {
				if ( ! is_array( $data ) || empty( $data['views'][$layout_id]['extension'] ) )
					return;
				
				$extension = $data['views'][$layout_id]['extension'];
				
				if ( '//DISABLE_EXTENSION//' == $extension ) {
					$this->_active_extension = $extension;
					return;
				}
				
				$disable_style = ( isset( $data['views'][$layout_id]['extension_data']['disable_theme_style'] ) ) ?  $data['views'][$layout_id]['extension_data']['disable_theme_style'] : false;
			}
			else
				return;
