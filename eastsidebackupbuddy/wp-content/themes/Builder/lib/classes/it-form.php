...
if ( ! class_exists( 'ITForm' ) ) {
	...
	class ITForm {
		...
		function get_post_data( $compact_used_inputs = false ) {
			$data = array();
		...
		function _get_simple_input( $var, $options, $override_value ) {
			...
			if ( ( false === $override_value ) && isset( $this->_options[$var] ) ) {
				if ( in_array( $options['type'], array( 'checkbox', 'radio' ) ) ) {
					if ( ( is_array( $this->_options[$var] ) && in_array( $options['value'], $this->_options[$var] ) ) || ( (string) $this->_options[$var] === (string) $options['value'] ) )
