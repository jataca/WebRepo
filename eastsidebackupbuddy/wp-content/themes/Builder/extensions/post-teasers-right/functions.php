...
function builder_teasers_right_extension() {
	$base_url = ITFileUtility::get_url_from_file( dirname( __FILE__ ) );
	
	add_filter( 'excerpt_length', 'builder_teasers_right_excerpt_length' );
	add_filter( 'excerpt_more', 'builder_teasers_right_excerpt_more' );
				...
				<?php while ( have_posts() ) : // the loop ?>
					...
					<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						<div class="entry-content teasers">
							...
							<?php else : ?>
								<?php edit_post_link( '<img width="150" height="200" src="' . $base_url . '/images/no-feature-image.jpg" class="alignright teaser-thumb no-thumb" />', '<div class="teasers-thumb-wrapper">', '</div' ) ; ?>
							<?php endif; ?>
...
<?php
	...
	remove_filter('excerpt_length', 'builder_teasers_right_excerpt_length');
	remove_filter('excerpt_more', 'builder_teasers_right_excerpt_more');
}

// How about a teaser lenght of 60.
function builder_teasers_right_excerpt_length( $length ) {
	return 60;
}

// Let's make the read more similar to the default.
function builder_teasers_right_excerpt_more( $more ) {
	global $post;
...
if ( ! is_singular() ) {
	add_action( 'builder_layout_engine_render', 'builder_teasers_right_extension_change_render_content', 0 );
}
function builder_teasers_right_extension_change_render_content() {
	remove_action( 'builder_layout_engine_render_content', 'render_content' );
	add_action( 'builder_layout_engine_render_content', 'builder_teasers_right_extension' );
}
