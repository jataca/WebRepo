...
if ( !function_exists( 'it_builder_slider_filter_image_downsize' ) && ! is_admin() ) {
	add_filter( 'image_downsize', 'it_builder_slider_filter_image_downsize', 10, 3 ); // Latch in when a custom image size is called.
	add_filter( 'intermediate_image_sizes_advanced', 'it_builder_slider_filter_image_downsize_blockextra', 10, 3 ); // Custom image size blocker to block generation of thumbs for sizes other sizes except when called.
	function it_builder_slider_filter_image_downsize( $result, $id, $size ) {
	...
	function it_builder_slider_filter_image_downsize_blockextra( $sizes ) {
		global $_ithemes_temp_downsize_size;
